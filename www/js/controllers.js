...
.controller('ClassCtrl', function($scope, Class,Loader){
 ...
 Loader.showLoading("Loading Jobs...");
...
//callback method finish
      ...
      for(var i=0;i<favClasses.length;i++){
        if(favClasses[i].id===$scope.classDetail.id){
          ...
          Loader.hideLoading();
          Loader.toggleLoadingWithMessage('jobs Already In Your favourites', 1000);
          console.log("this job already exist");
          favClassexist=true;
           break;
        }
      }
    ...
    var favRef = new Firebase("https://getjob.firebaseio.com/users/"+userId+"/favClass");
     if(!favClassexist){
      favRef.push().set({
...
al.userId= AuthFactory.getUser();
  ...
  //doing the logout
   al.logout= function(){
    var ref = new Firebase("https://getjob.firebaseio.com/");
     ref.unauth();
...
var adl = this;
  ...
  var refClass = new Firebase("https://getjob.firebaseio.com/class");
  ...
  {
  Loader.showLoading('Adding jobs..');
...
//callback method finish
      ...
      for(var i=0;i<totalClasses.length;i++){
        if(adl.classId===totalClasses[i].id){
          console.log("looprunning");
          Loader.hideLoading();
          Loader.toggleLoadingWithMessage('Jobs With id '+adl.classId+' Already Exist ', 1000);
          console.log("this job already exist");
          favClassexist=true;
           break;
        }
   ...
   vm.createUser = function(){
    ...
    Auth.$createUser({
        ...
        console.log(userData);
         Loader.toggleLoadingWithMessage("User created successfully " , 1000);
